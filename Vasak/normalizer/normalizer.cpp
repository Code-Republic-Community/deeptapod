/** this file contains implementations of functions that`
 
 * check whether the symbol is number, letter, etc.
 * split sentence into words
 * check whether the word is in the database or not (whether has meaning or not)
 * normalize the word to keep the direct form
...
bool is_apostrophe(const char& symbol) {
	return (symbol == 39) ? true : false;
}

bool is_end_of_word(const char& symbol) {
	return (symbol == '\0' || symbol == ' ' || symbol == 9 || symbol == 11) ? true : false;
}
...
std::vector<std::string> split_text_into_words(const std::string& text){
    ...
    for (int i = 0; i <= text.size(); ++i) {
	    if (i > 0 && i < text.size()-1 && (is_apostrophe(text[i]) && is_letter(text[i-1]) && is_letter(text[i+1]))) {
		    ...
		    strWord += text[i];
	    } else if (i == 0 && i < text.size() && text[i] == '0' && is_letter(text[i+1])) {
	    	    strWord += 'o';
